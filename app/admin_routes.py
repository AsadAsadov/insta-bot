...
def _flash_params(message: str, level: str) -> dict[str, str]:
    return {"flash": message, "flash_type": level}
...
def thread_detail(request: Request, thread_id: str):
    ...
    context = {
        **_base_context(request),
        ...
        "latest_failed": db.row_to_dict(latest_failed) if latest_failed else None,
    }
    return templates.TemplateResponse("thread.html", context)
